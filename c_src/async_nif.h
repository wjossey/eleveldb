...
static int async_nif_size_thread_pool(unsigned int amt)
{
  ...
  if (amt == 0) {
    ...
    if (async_nif_worker_count == 0) {
      /* Create a new pool. */
      async_nif_worker_entries = (struct async_nif_worker_entry *)enif_alloc(sizeof(struct async_nif_worker_entry) * amt);
    } else {
      /* Growing the thread pool from `async_nif_worker_count` by `amt` new workers. */
      struct async_nif_worker_entry *new_entries = (struct async_nif_worker_entry *)enif_realloc(async_nif_worker_entries, sizeof(struct async_nif_worker_entry) * amt);
      async_nif_worker_entries = new_entries; /* It could have moved, but that should be ok. */
    }
